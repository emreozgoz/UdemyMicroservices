...
namespace FreeCourse.Services.Catalog.Services
{
    public class CourseServices : ICourseService
    {
        ...
        public async Task<Response<CourseDto>> GetById(string id)
        {
            ...
            course.Category = await _categoryCollection.Find<Category>(x => x.Id == course.Category.Id).FirstAsync();
            return Response<CourseDto>.Success(_mapper.Map<CourseDto>(course), 200);
        }

        public async Task<Response<List<CourseDto>>> GetAllUserByIdAsync(string userId)
        {
            var courseWithUserId = await _courseCollection.Find(x => x.UserId == userId).ToListAsync();
            if (courseWithUserId.Any())
            {
                foreach (var course in courseWithUserId)
                {
                    course.Category = await _categoryCollection.Find<Category>(x => x.Id == course.CategoryId).FirstAsync();
                }
            }
            ...
            {
                courseWithUserId = new List<Course>();
            }
            return Response<List<CourseDto>>.Success(_mapper.Map<List<CourseDto>>(courseWithUserId), 200);
        }
